...
if (isset($_POST['user']) && isset($_POST['pass'])) {
	...
	$len = $Mysql -> RowCount("px_members", "  where `email` = '$username' and `password` = '$password'  and `Admin` ='1'  ");
 
	if ($len != 0) {
		//found return data

		$data = $Mysql -> Query(" select * from `px_members`   where `email` = '$username' and `password` = '$password' and `Admin` ='1'  ");

		$userInfo = $data[0];


		$result = array('login' => 'success', 'data' => $userInfo);
	 	 $_SESSION['Login_admin'] = $result;
		 session_regenerate_id(true);
